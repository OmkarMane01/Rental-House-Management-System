...
if(isset($_POST['submit']))
{
     ...
     if(!empty($name) && !empty($middlenm) && !empty($lastnm) && !empty($city) && !empty($area) && !empty($hid) && !empty($add) && !empty($floorno) && 
      !empty($formatofflat) && !empty($email)  && !empty($mobno) && !empty($desc) &&  !empty($folder)  &&  !empty($folder2)  &&  !empty($rent))
     {
          ...
          {   //basic firstname validation
            ...
            echo "<script type='text/javascript'>alert('Firstname must have atlaest 4 characters!')
                ...
                echo "<script language='javascript' type='text/javascript'>location.href='addflat.php'
               </script>";
          }
          else 
          ...
          {   
           ...
           //houseid validation
            if(!preg_match("/^[0-9]+$/",$hid))
          {  
             $error=true;
            echo "<script type='text/javascript'>alert('House ID must contain numbers!')
               </script>";
          ...
          {   
            ...
            echo "<script type='text/javascript'>alert('Address must have atlaest 20 characters!')
              ...
              //houseid validation
            if(!preg_match("/^[0-9]+$/",$floorno))
          {  
             $error=true;
            echo "<script type='text/javascript'>alert('Floor number must contain numbers!')
               </script>";
    ...
    //description  validation 
          ...
          {  
             $error=true;
            echo "<script type='text/javascript'>alert('rent rupees must contain numbers!')
               </script>";
...
VALUES (
'$name', '$middlenm', '$lastnm',  '$city',  '$area',  '$hid',  '$add',  '$floorno',  '$formatofflat',  '$email', 
 '$mobno',  '$desc',  '$folder','$folder2','$rent')";
                      ...
                      if($is_query_run=mysqli_query($conn,$query))
                       {  //Insert into database Query
                          // echo " query executed";
                         echo "<script type='text/javascript'>alert('YOUR ADD FLAT WAS DONE SEUCCESFULLY!')
                         </script>";
